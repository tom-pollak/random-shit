...
def optimize(selections: list, existing_bets: list, bankroll: float):
    ...
    for index_selection, selection in enumerate(selections):
        for existing_bet in existing_bets:
            ...
            if existing_bet["name"] == selection["name"]:
                existing_bets_profits[index_selection] += (
                    existing_bet["stake"] * existing_bet["odds_book"]
                )
    ...
    def f(stakes):
        ...
        end_bankrolls, objective = (
            len(selections) * [bankroll - np.sum(stakes)]
            + np.asarray(existing_bets_profits),
            0.00,
        )
        ...
        for index_selection, selection in enumerate(selections):
            ...
            # ADD WINS & LOSSES
            end_bankrolls[index_selection] += (
                stakes[index_selection] * selection["odds_book"]
            )
    ...
    res = scipy.optimize.differential_evolution(
        func=f, bounds=bounds, constraints=(nlc)
    )
    ...
    for num, selection in enumerate(selections):
        stake = round(res.x[num], 2)
        if stake > 0:
            print(
                f"{selections[num]['name']} @{selections[num]['odds_book']} - € {stake}"
            )


selections = list()
selections.append({"name": "Win", "odds_book": 2, "odds_fair": 5})
selections.append({"name": "Place", "odds_book": 7, "odds_fair": 5})
# selections.append({"name": "Lose", "odds_book": 10, "odds_fair": 1 / 3})
# selections.append(
#     {"name": "Borussia Monchengladbach", "odds_book": 201, "odds_fair": 1176.471}
# )
...
# existing_bets.append({"name": "FC Koln", "odds_book": 3, "stake": 35})

# CALL THE FUNCTION WITH GIVEN BANKROLL
optimize(selections=selections, existing_bets=existing_bets, bankroll=300)
